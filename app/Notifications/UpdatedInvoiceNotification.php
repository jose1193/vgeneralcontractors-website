...
class UpdatedInvoiceNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if ($this->invoice->pdf_url) {
            $mailMessage->line(new HtmlString('<div style="margin-top: 15px; margin-bottom: 15px;">'))
                ->line('You can also view or download the updated invoice PDF:')
    ...
    {
        if ($this->isInternal) {
            return "Invoice Updated - {$this->invoice->invoice_number} - {$this->invoice->bill_to_name}";
        }

        return "Updated Invoice #{$this->invoice->invoice_number} from {$this->companyData->company_name}";
    }
}