...
class AppointmentController extends BaseCrudController
{
    ...
    /**
     * Validate a single field via AJAX (similar to FacebookLeadFormController)
     */
    public function validateField(Request $request)
    {
        try {
            ...
            $excludeUuid = $request->input('exclude_uuid'); // For edit mode

            if (!$fieldName) {
                return response()->json(['error' => 'Field name not provided.'], 400);
            }

            $rules = $this->getValidationRules($excludeUuid);
            ...
            if ($validator->fails()) {
                ...
                // Check specifically for email field and if it already exists in the database
                if ($fieldName === 'email' && !empty($fieldValue)) {
                    $existingEmail = Appointment::where('email', $fieldValue);
                    
                    // Exclude current appointment when editing
                    if ($excludeUuid) {
                        $existingEmail->where('uuid', '!=', $excludeUuid);
                    }
                    
                    if ($existingEmail->withTrashed()->exists()) {
                        return response()->json([
                            'valid' => false, 
            ...
            Log::error("Error validating field: {$e->getMessage()}", [
                'exception' => $e,
                'request' => $request->all(),
            ]);
