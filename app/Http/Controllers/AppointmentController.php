...
class AppointmentController extends BaseCrudController
{
    ...
    /**
     * Validate a single field via AJAX for real-time validation
     */
    public function validateField(Request $request)
    {
        $fieldName = $request->input('fieldName');
        $fieldValue = $request->input('fieldValue');
        $excludeUuid = $request->input('excludeUuid'); // For edit mode
        ...
        $allRules = $this->getValidationRules($excludeUuid);
        $allMessages = $this->getValidationMessages();

        if (!isset($allRules[$fieldName])) {
            return response()->json(['error' => 'Invalid field name.'], 400);
        }

        // Get rules and messages specific to this field
        $fieldRules = [$fieldName => $allRules[$fieldName]];
        $fieldMessages = array_filter($allMessages, function($key) use ($fieldName) {
            return strpos($key, $fieldName . '.') === 0;
        }, ARRAY_FILTER_USE_KEY);

        // Special handling for insurance_property radio buttons
        if ($fieldName === 'insurance_property') {
            $fieldValue = $fieldValue === '1' ? 1 : 0;
        }

        $validator = Validator::make([$fieldName => $fieldValue], $fieldRules, $fieldMessages);
        ...
        if ($validator->fails()) {
            ...
            // Special validation for email field - check if it already exists
            if ($fieldName === 'email' && !empty($fieldValue)) {
                $query = Appointment::where('email', $fieldValue);
                ...
                $existingEmail = $query->withTrashed()->exists();
                
                if ($existingEmail) {
                    return response()->json([
                        'valid' => false, 
                        'errors' => [__('email_unique')],
