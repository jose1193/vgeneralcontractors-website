...
@section('content')
    ...
    <div class="min-h-screen py-8" x-data="invoiceDemoData()" x-init="init()">
        ...
        @push('scripts')
            ...
            <!-- Debug Script -->
            <script>
                // Debug function to test date picker manually
                window.testDatePicker = function() {
                    console.log('🧪 Manual date picker test...');
                    const element = document.querySelector('#dateRangePicker');
                    if (element) {
                        try {
                            const picker = new ModernDateRangePicker({
                            ...
                            console.log('✅ Manual date picker created successfully:', picker);
                            return picker;
                        } catch (error) {
                            console.error('❌ Manual date picker creation failed:', error);
                        }
                    } else {
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    console.log('🔍 DOM Content Loaded - Starting diagnostics...');
                    
                    // Check if ModernDateRangePicker is available
                    if (typeof ModernDateRangePicker !== 'undefined') {
                        console.log('✅ ModernDateRangePicker class is available');
                    } else {
                        console.error('❌ ModernDateRangePicker class not found');
                    }
                    
                    // Check if Litepicker is available
                    if (typeof Litepicker !== 'undefined') {
                        console.log('✅ Litepicker is available');
                    } else {
                        console.log('⚠️ Litepicker not available (will be loaded dynamically)');
                    }
                    
                    // Check if date picker element exists
                    ...
                    if (datePickerElement) {
                        ...
                        console.error('❌ Date picker element #dateRangePicker not found');
                    }
                    
                    // Check Alpine.js data
                    setTimeout(() => {
                        const alpineData = window.Alpine ? window.Alpine.store : null;
                        console.log('🔍 Alpine.js available:', !!window.Alpine);
                        
                        // Try to access the invoice demo component
                        const invoiceContainer = document.querySelector('[x-data*="invoiceDemo"]');
                        if (invoiceContainer) {
                            console.log('✅ Invoice demo container found');
                        } else {
                            console.error('❌ Invoice demo container not found');
                        }
                    }, 500);
                });
                ...
                document.addEventListener('alpine:initialized', () => {
                    ...
                    // Try to manually trigger date picker initialization after Alpine is ready
                    setTimeout(() => {
                        console.log('🔄 Attempting manual date picker initialization...');
                        if (typeof window.testDatePicker === 'function') {
